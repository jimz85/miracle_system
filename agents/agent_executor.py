    ...
    def __init__(self, encryption_key_path: Optional[str] = None):
        ...
        if self.encryption_key:
            try:
                ...
                logging.warning("cryptography库未安装，加密功能不可用")
        self._keys_cache = {}
...
class SlippageMonitor:
    ...
    def record_execution(self, signal: Dict, planned_price: float, actual_price: float) -> Dict:
        ...
        slippage_pct = slippage_abs / planned_price
...
class TradeLogger:
    ...
    def log_entry(self, trade_record: Dict) -> str:
        ...
        logging.info(f"入场记录: {trade_id} {record['symbol']} {record['side']} @ {record['entry_price']}")
        return trade_id
    ...
    def log_signal_miss(self, signal: Dict, reason: str):
        ...
        logging.warning(f"信号放弃 [{signal.get('symbol')}]: {reason}")
    ...
    def _send(self, message: Dict, color: str = "blue"):
        ...
        try:
            ...
            if result.get("code") != 0:
                logging.warning(f"飞书API返回错误: {result}")
            else:
                logging.info(f"飞书通知发送成功")
