...
> **架构说明（2026-04-28）：** 以下为设计目标架构。实际实现为 3 个核心 Agent：
> - `AgentSignal`（信号生成）：PriceFactors + 多因子融合 + 白名单过滤
> - `AgentExecutor`（执行引擎）：OKX下单 + 熔断 + 动态止损
> - `AgentLearner`（学习迭代）：IC权重更新 + 模式黑名单
> Agent-M（市场情报）→ 内化为 `ExchangeClient` 数据获取模块
> Agent-R（风险管理）→ 内化为 `CircuitBreaker` + `RiskMetrics` 模块
> Orchestrator → 由 `scripts/miracle_kronos.py` 中的 `run_scan()` 实现

```
┌─────────────────────────────────────────────────────────────────────────┐
│                        Miracle System 自主学习系统                        │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│   ┌─────────────────────────────────────────────────────────────┐     │
│   │                    run_scan() (编排器)                       │     │
│   │              主控制循环：扫描→评分→决策→执行→复盘            │     │
│   └─────────────────────────────────────────────────────────────┘     │
│                              │                                          │
│          ┌───────────────────┼───────────────────┐                   │
│          ▼                   ▼                   ▼                   │
│   ┌─────────────┐     ┌─────────────┐     ┌─────────────┐            │
│   │  市场数据   │     │  Agent-S   │     │  Agent-L    │            │
│   │  采集模块   │────▶│  信号生成   │────▶│  学习迭代   │            │
│   │(ExchangeClient)│   │(agent_signal)│    │(agent_learner)│         │
│   └─────────────┘     └─────────────┘     └─────────────┘            │
│          │                   │                   │                    │
│          │                   ▼                   │                    │
│          │           ┌─────────────┐            │                    │
...
| 设计Agent | 状态 | 实际实现 |
|-----------|------|---------|
| Agent-M（市场情报） | ✅ 已内化 | `core/exchange_client.py` 数据获取 |
| Agent-S（信号生成） | ✅ 已实现 | `agents/agent_signal.py::AgentSignal` |
| Agent-R（风险管理） | ✅ 已内化 | `core/circuit_breaker.py` + `core/risk_metrics.py` |
| Agent-E（执行引擎） | ✅ 已实现 | `agents/agent_executor.py::AgentExecutor` |
| Agent-L（学习迭代） | ✅ 已实现 | `agents/agent_learner.py::AgentLearner` |
| Orchestrator | ✅ 已实现 | `scripts/miracle_kronos.py::run_scan()` |
| Multi-Agent Debate | ⏸️ 架构预留 | Roadmap 中标注，暂未实现 |
│                                                                         │
│   ┌─────────────────────────────────────────────────────────────┐     │
│   │                    Tool Ecosystem (工具生态)                   │     │
│   │  ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐         │     │
...
> **说明（2026-04-28）：** 文档中的"Agent-M/S/R"职责已重新映射为实际组件。

1. 分析市场情报（ExchangeClient 数据采集产出）
2. 评估交易信号（AgentSignal 产出）
3. 审核风险管理（CircuitBreaker + RiskMetrics 反馈）
4. 决定是否执行
5. 从每笔交易中学习（AgentLearner 迭代）
